//! easyVPN engine (GUI-agnostic).
//!
//! Turns the host Mac into an obfuscated VPN server that stands up to
//! aggressive network censorship. It drives a local `sing-box` install with
//! two inbounds on port 443:
//!
...
//!
//! This crate contains no UI and no Tauri dependency, so it is shared by both
//! the desktop app (`src-tauri`) and the CLI (`easyvpn`). State lives under
//! `~/Library/Application Support/com.me.easyvpn/`, so the two front-ends
...
/// How a privileged (root) operation should acquire its privileges.
#[derive(Clone, Copy, Debug)]
pub enum Elevation {
    /// Pop a native macOS admin password dialog (used by the desktop app).
    GuiPrompt,
    /// Assume the process is already running as root (used by `sudo easyvpn`).
    AlreadyRoot,
}
...
pub fn is_root() -> bool {
    run("id", &["-u"]).map(|u| u.trim() == "0").unwrap_or(false)
}

/// Run a script with root privileges. With [`Elevation::GuiPrompt`] this shows
/// a single native macOS password dialog; with [`Elevation::AlreadyRoot`] it
/// runs directly (the caller must already be root).
fn run_privileged(script: &str, prompt: &str, mode: Elevation) -> Result<String, String> {
    match mode {
...
fn render_config(state: &ServerState) -> serde_json::Value {
    let vless_users: Vec<serde_json::Value> = state
        .guests
        .iter()
        .map(|g| serde_json::json!({ "uuid": g.uuid, "flow": "xtls-rprx-vision" }))
        .collect();
...
// ---------------------------------------------------------------------------
// Public API — the operations both front-ends call
// ---------------------------------------------------------------------------
...
/// Create the server identity. Idempotent: never clobbers an existing one.
...
pub fn add_guest(name: &str) -> Result<Guest, String> {
...
pub fn remove_guest(name: &str) -> Result<(), String> {
    let mut state = load_state().ok_or("create the server first")?;
...
/// Requires root (binding :443 and installing a system LaunchDaemon).
pub fn apply_and_start(mode: Elevation) -> Result<String, String> {
    let state = load_state().ok_or("create the server first")?;
...
"#,
    ...
    run_privileged(
        &script,
        "easyVPN needs administrator access to start the VPN service",
        mode,
    )?;
...
pub fn stop_server(mode: Elevation) -> Result<String, String> {
    let script = format!(
...
"#,
    ...
    run_privileged(
        &script,
        "easyVPN needs administrator access to stop the VPN service",
        mode,
    )?;
...
mod tests {
    ...
    fn vless_link_has_reality_params() {
        ...
        assert!(links
            .vless_link
            .starts_with("vless://11111111-2222-3333-4444-555555555555@1.2.3.4:443"));
    ...
    fn hysteria2_link_has_obfs_and_is_url_safe() {
        ...
        assert!(links
            .hysteria2_link
            .starts_with("hysteria2://hy2pw@1.2.3.4:443"));
